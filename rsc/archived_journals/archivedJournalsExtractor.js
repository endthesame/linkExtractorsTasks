...
async function crawlPages(startUrl) {
    ...
    while (true) {
        ...
        fs.appendFileSync('found_links_rsc_archived.txt', contentLinks.join('\n') + '\n');
        console.log(`Links from Page ${currentPage} have been saved to found_links.txt!`);

        // Проверяем наличие кнопки paging__btn--next
        const nextPageButton = await page.$('.article-nav__bar li a');
        ...
        try {
            ...
            await page.click('.article-nav__bar li a');
        } catch (error) {
...
async function main() {
    const sourceLinksPath = 'links_to_crawl_current.txt';
