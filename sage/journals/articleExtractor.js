...
async function crawlPages(startUrl, page) {
    ...
    if (await isCookieAcceptNeeded(page)) {
        try {
            ...
            logMessage("cannot accept cookie");
        }
    }
    ...
    try {
        ...
        logMessage("cant click current issue - BREAK THIS LINK");
        return;
    }
    ...
    while (true) {
        ...
        logMessage(`Links from Page ${currUrl}; links length: ${contentLinks.length}`);
        ...
        try {
            ...
            if (await isNextButtonExist(page)) {
                ...
                logMessage("no prev button");
                break;
            }
        } catch (error) {
            logMessage(`Failed to click the prev page button. Error: ${error.message}`);
...
async function main() {
    ...
    for (const sourceLink of sourceLinks) {
        logMessage(`Crawling pages for source link: ${sourceLink}`);
