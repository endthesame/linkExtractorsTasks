...
async function extractLinks(page) {
    return await page.evaluate(() => {
        let rawChaptersLinks =  Array.from(document.querySelectorAll('.article-list .tile-grid-item a'))
            .map(link => link.href);
...
async function crawlPages(startUrl, page) {
    await page.goto(`${startUrl}/contents`, { waitUntil: 'networkidle2', timeout: 50000 });

    await page.waitForTimeout(1000);

    const contentLinks = await extractLinks(page);

    fs.appendFileSync('found_links_ems_chapters.txt', contentLinks.join('\n') + '\n');
    console.log(`Links from Page ${startUrl} have been saved to found_links.txt!`);

}

async function main() {
    const sourceLinksPath = 'links_to_crawl_chapters.txt';
