...
async function choosePage(page, issue_button, volume_button){
    let links = []
    if (issue_button){
        await issue_button.click({ waitUntil: 'networkidle2', timeout: 50000 });
        // цикл по всем годам
        let extractedLinks = await extractIssueLinks()
        links = [...links, ...extractedLinks];
    }
    else if (volume_button) {
        await volume_button.click({ waitUntil: 'networkidle2', timeout: 50000 });
    }
    else {
...
async function crawlPages(startUrl, page) {
    ...
    await choosePage(page, all_issue_button, all_volume_button);
