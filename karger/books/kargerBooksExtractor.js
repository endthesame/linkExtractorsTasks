...
async function extractLinks(page) {
    return await page.evaluate(() => {
        return Array.from(document.querySelectorAll('a[href]'))
            .filter(a => a.href.match(/.*chapter\/.*searchresult.*$/))
            .map(link => link.href);
    });
}
...
async function crawlPages(startUrl, page) {
    ...
    for(currentPage; currentPage < 3700; currentPage++) {
        ...
        fs.appendFileSync('found_links_karger_books.txt', contentLinks.join('\n') + '\n');
