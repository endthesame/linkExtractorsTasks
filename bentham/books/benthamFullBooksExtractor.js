...
async function extractLinks(page) {
    return await page.evaluate(() => {
        let rawLinks =  Array.from(document.querySelectorAll('a[href]'))
            .map(elem => elem.href)
            .filter(link => link.includes("ebook_volume"))
...
async function crawlPages(startUrl, page) {
    ...
    for(currentPage; currentPage < 125; currentPage++) {
        ...
        fs.appendFileSync('found_links_bentham_fullbooks.txt', contentLinks.join('\n') + '\n');
        console.log(`Links from Page ${currUrl}; links length: ${contentLinks.length}`);
        try {
            await page.waitForSelector('.pagination');
            // Попытка клика на кнопку paging__btn--next
            await page.click('.page-link[rel="next"]', { waitUntil: 'networkidle2', timeout: 50000 });
        } catch (error) {
